...
function param = Hornberg2005EGFRsignalling_species2params(model)
...
% number of reactions
param.nreact = 148; % this needs to be manually supplied 
...
r91 = 'par(I.kd91) * X(I.MEKi_PP_Pase2i)- par(I.k91) * X(I.MEKi_PP) * X(I.Phosphatase2)';
...
for k = 1:I.nstates
        ...
        if contains(ratestring,statestring)
            ...
            for fb = 1:length(fbrates)
                ...
                if bothreactions || contains(fbrates{fb},statestring)
                    ...
                    states2param = [states2param,I.(nmparam{:})];
                end
            end
    ...
    param.states2Ipar{k} = states2param;
    param.states2nmpar{k} = [I.nmpar(states2param)];
end
